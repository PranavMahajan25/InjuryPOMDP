...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~# 
    ...
    def belief_transition_matrix_plot(self,figSave):
            ...
            ax[0].plot(self.obvs,np.transpose(self.obvPrior)[:,0], 'dimgray',linewidth=8, label="$P(o|s=0, a_{nul})$")
            ax[0].plot(self.obvs,np.transpose(self.obvPrior)[:,1], 'black',linewidth=8, label="$P(o|s=1, a_{nul})$")
            # ax[0].set(ylim=[0,0.1],xlim=[0,100],title=self.name,xlabel='Observation estimate', ylabel='Probability')
            ax[0].set(ylim=[0,0.05],xlim=[0,100])
            ...
            ax[1].set(xlabel='Next belief state ($b\'$)',ylabel='Current belief state ($b$)')
            fig.colorbar(im, ax=ax[1], label='P($b\'$|$b$)')
            ax[1].set(xticks = [0, 50, 100], xticklabels=[0, 0.5, 1], yticks = [0, 50, 100], yticklabels=[0, 0.5, 1])
            ...
            if figSave != "": #...print individual to file
                ...
                plt.savefig(subPath+'belief_transition'+str(self.name)+'.png', dpi=1500)
            #    plt.savefig(subPath+'belief_transition'+str(self.name)+'.svg')
                plt.close(fig)
            return [self.obvs, np.transpose(self.obvPrior)]
            ...
            for c,cost in enumerate(allCosts):
                ...
                while updateTrue:    
                    ...
                    for b,belief in enumerate(beliefRange): #...for each belief
                        ...
                        # changed to belief dependent rho(a_que)
                        qSample = qVal_cost_belief_dependent_a_que(cost,belief,bPrime,v,self.obvStep,'-') 

                        qL,qR,_ = qVal_lr(rewards,belief,'-') 
                        # print(belief, qL, qR, qSample)
                        actionValues[b,:] = [qL,qR,qSample]
                        
                        beliefValues[b] = belief_values(actionValues[b,:])
                        # print(actionValues[b,:], beliefValues[b])
